...
redisRawStreamingDF = spark.readStream\
    .format("kafka")\
...
redisEncodedStreamingDF = spark.sql("select key, zSetEntries[0].element as customer from RedisSortedSet")
...
redisDecodedStreamingDF = redisEncodedStreamingDF.withColumn("customer", unbase64(redisEncodedStreamingDF.customer).cast("string"))

# TO-DO: parse the JSON in the Customer record and store in a temporary view called CustomerRecords
redisDecodedStreamingDF\
...
# TO-DO: cast the value column in the streaming dataframe as a STRING 
stediStreamingDF = stediRawStreamingDF.selectExpr("cast(key as string) as key", "cast(value as string) as value")
...
# customerRiskJoinSelectedStreamingDF.writeStream\
#     .outputMode("append")\
#     .format("console")\
#     .start()\
#     .awaitTermination()
...
customerRiskJoinStreamingDF.selectExpr("cast(customer as string) as key", "to_json(struct(*)) as value")\
    ...
    .option("checkpointLocation", "/tmp/customer-risk-checkpoint-1")\
    .start()\
    .awaitTermination()
